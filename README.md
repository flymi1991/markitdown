...
2. Current Python environment directory: `<python-prefix>/share/markitdown/models`.
3. Current Python environment subdirectory: `<python-prefix>/share/markitdown/models/SenseVoiceSmall`.
4. Package-local model directory, if packaged with the installation.
5. Source-tree model directory: `packages/markitdown/models/SenseVoiceSmall`.
6. ModelScope model name: `iic/SenseVoiceSmall`.

After installing the package, install/copy the SenseVoice model files into the active Python environment:

```bash
markitdown --install-sensevoice-model
```

Or pass a downloaded SenseVoice model directory explicitly:

```bash
markitdown --install-sensevoice-model "D:/models/SenseVoiceSmall"
```

After this step, `markitdown.exe` can be run from any working directory without depending on the source project path.
...
The `markitdown-ocr` plugin adds OCR support for standalone JPG/PNG images and for images embedded in PDF, DOCX, PPTX, and XLSX files. It uses the `ocr_llm` section in `markitdown_config.json`. No new ML libraries or binary dependencies required.
...
Configure `ocr_llm`, then enable plugins:

```bash
markitdown document.pdf --use-plugins -o document.md
markitdown image.jpg --use-plugins -o image.md
```
...
If `ocr_llm` is not configured, the plugin still loads, but OCR is silently skipped and the standard built-in converter is used instead.
